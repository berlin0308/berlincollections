...
4  942U0120    00     微波積體電路專題        林坤佑     電二101  四7,8,9
    ...
    def transformTime(self, time):
        ...
        if(periods[0] == periods[1]):  # only one session
            ...
            end = self.periodKey.index(periods[1])-1 #原本有+1但我不太懂
            return_str = ','.join(self.periodDict[k]
                                  for k in self.periodKey[start:end+2])
            print("day:"+str(day)+"  periods:"+str(periods)+"  start:"+str(start)+"  end:"+str(end))
